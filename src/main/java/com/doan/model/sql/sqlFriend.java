...
public class sqlFriend {
    ...
    // try {
    // Connection connection = sqlConnect.connectToDB();
    // String sql = "SELECT COUNT(*) AS common_friends FROM friend WHERE (userID1 =
    // ? AND userID2 = ?) OR (userID1 = ? AND userID2 = ?)";
    // PreparedStatement preparedStatement = connection.prepareStatement(sql);
    // preparedStatement.setString(1, userId);
    // preparedStatement.setString(2, userId);
    // preparedStatement.setString(3, userId);
    ...
    // // Trích xuất thông tin về bạn bè từ ResultSet và thiết lập cho đối tượng
    // friend
    ...
    // Lấy ra danh sách các userId có tên giống keyword và đã kết bạn với userId truyền vào
    public static ArrayList<String> getListUserIdFriendhasNameLikeKeyword(String userId, String keyword) {
        ArrayList<String> list = new ArrayList<>();
        try {            
            String sql = "SELECT DISTINCT userID ";
            sql += "FROM `account_details` ad ";
            sql += "JOIN `friend` f ON ad.`userID` = f.`userID1` OR ad.`userID` = f.`userID2` ";
            sql += "WHERE (LOWER(ad.`firstName`) LIKE LOWER('%" + keyword + "%') ";
            sql += "OR LOWER(ad.`lastName`) LIKE LOWER('%" + keyword + "%'))";
            sql += "AND (f.`userID1` = '" + userId + "' OR f.`userID2` = '" + userId + "') ";
            sql += "AND userID != '" + userId + "'  AND f.`deleted` = 0";
    ...
    public static ArrayList<AccountDetails> getAccountDetailsListByKeyword(String userId, String keyword) {
        ArrayList<AccountDetails> list = new ArrayList<>();
        try {            
            String sql = "SELECT DISTINCT userID, firstName, lastName, address, birthDate, avatarName, background, chatID ";
            sql += "FROM `account_details` ad ";
            sql += "JOIN `friend` f ON ad.`userID` = f.`userID1` OR ad.`userID` = f.`userID2` ";
            sql += "WHERE (LOWER(ad.`firstName`) LIKE LOWER('%" + keyword + "%') ";
            sql += "OR LOWER(ad.`lastName`) LIKE LOWER('%" + keyword + "%'))";
            sql += "AND (f.`userID1` = '" + userId + "' OR f.`userID2` = '" + userId + "') AND userID != '" + userId + "' AND f.`deleted` = 0";
           ...
           while (rs.next()) {
                AccountDetails friendDetails = new AccountDetails();
                // Trích xuất thông tin từ account_details và thiết lập cho đối tượng friendDetails
                friendDetails.setUserID(rs.getString("userID"));
                friendDetails.setFirstName(rs.getString("firstName"));
                friendDetails.setLastName(rs.getString("lastName"));
                friendDetails.setAddress(rs.getString("address"));
                friendDetails.setBirthDate(rs.getDate("birthDate"));
                friendDetails.setAvatar(rs.getString("avatarName"));
                friendDetails.setBackground(rs.getString("background"));
                friendDetails.setChatID(rs.getInt("chatID"));
                
                // Thêm friendWithChatID vào danh sách
                list.add(friendDetails);
            }
            statement.close();
            connection.close();
        } catch (Exception e) {
