...
<div class="post-card" id="post-card-${postItem.post.postID}">
	...
	<input type="hidden" name="pictureQuantity"
		value="${postItem.post.postID}"> <input type="hidden"
		name="likedQuantity" id="likedQuantity-${postItem.post.postID}"
		value="${postItem.post.likeCount}"> <input type="hidden"
		name="isLiked-${postItem.post.postID}"
		id="isLiked-${postItem.post.postID}" value="${postItem.isLikedByUser}">
	<div class="top">
		<a href="#">
			<div class="user-img">
				<img
					src="/stored-user-images/${userDetails[postItem.post.userID].avatar}"
					alt="">
			</div>
		</a>
		<div class="user-info">
				...
				<c:choose>
					<c:when test="${fn:length(postItem.images)==1}">
						<div class="pictures-line-1">
							<img src="/stored-user-images/${postItem.images[0].image}"
								alt="">
						</div>
					</c:when>
					<c:when test="${fn:length(postItem.images)==2}">
						<div class="post-image-item-small-wrapper">
							...
							<div class="pictures-line-1">
								<img src="/stored-user-images/${postItem.images[1].image}"
									alt="">
							</div>
						</div>
					</c:when>
					<c:when test="${fn:length(postItem.images)==3}">
						...
						<div class="pictures-line-1">
							<img src="/stored-user-images/${postItem.images[2].image}"
								alt="">
						</div>
					</c:when>
					<c:when test="${fn:length(postItem.images)==4}">
						...
						<div class="pictures-line-2">
							<img src="/stored-user-images/${postItem.images[3].image}"
								alt="">
						</div>
					</c:when>
					<c:when test="${fn:length(postItem.images)>4}">
	...
	<div class="interact-container">
		<div class="top">
			...
			<div id="commentNumberId-${postItem.post.postID}"
				class="comment-number"
				onclick="openPostCommentCard('${postItem.post.postID}');
		...
		<div class="bottom">
			<c:if test="${!postItem.isLikedByUser}">
				<div id="likePost-${postItem.post.postID}" class="container"
					onclick="; 
                    processLike('${postItem.post.postID}',this) ">
					<i class="fa-regular fa-thumbs-up fa-2x"></i> Like
			</c:if>
			<c:if test="${postItem.isLikedByUser}">
				<div id="likePost-${postItem.post.postID}" class="container liked"
					onclick=";
                    processLike('${postItem.post.postID}',this)">
		...
		<div class="container" id="comment-section-${postItem.post.postID}"
			onclick="openPostCommentCard('${postItem.post.postID}');
                getPostDetail('${postItem.post.postID}')">
